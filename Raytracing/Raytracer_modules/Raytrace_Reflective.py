...
def compute_pixel(args):
    ...
    D = D / vecLength(D)
    color = TraceRay(O, D, 1, np.inf, spheres, lights, 3)
    return x + canvasWidth // 2, y + canvasHeight // 2, color
...
def ComputeLighting(P, N, V, s, lights):
    ...
    for light in lights:
        if light.type == "ambient":
            ...
            if shadow_sphere is not None:  # If any object blocks the light
                continue
